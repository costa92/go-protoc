...
readonly DATA_VOLUME_NAME="${CONTAINER_NAME_MONGODB}-data"
readonly CONFIG_VOLUME_NAME="${CONTAINER_NAME_MONGODB}-config"
readonly LOGS_VOLUME_NAME="${CONTAINER_NAME_MONGODB}-logs"
...
DATA_VOLUMES="${CONTAINER_NAME_MONGODB}-data,${CONTAINER_NAME_MONGODB}-config,${CONTAINER_NAME_MONGODB}-logs"
...
if [ "$should_remove_data" = true ]; then
    ...
    echo ""
    echo "   删除数据的方式："
    echo "     环境变量: REMOVE_DATA=true make docker.mongodb.stop"
    echo "     直接调用: bash <脚本路径> --remove-data"
    echo "     强制删除: REMOVE_DATA=true FORCE_DELETE=true make docker.mongodb.stop"
fi
