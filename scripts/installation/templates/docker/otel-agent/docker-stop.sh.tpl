...
readonly VOLUME_NAME="${CONTAINER_NAME_OTEL_AGENT}-data"

echo "正在停止OTEL Agent容器..."

# 停止容器
if docker stop "${CONTAINER_NAME}" 2>/dev/null; then
    echo "OTEL Agent容器已停止: ${CONTAINER_NAME}"
else
    echo "OTEL Agent容器未运行或已停止"
fi

# 删除容器
if docker rm "${CONTAINER_NAME}" 2>/dev/null; then
    echo "OTEL Agent容器已删除: ${CONTAINER_NAME}"
else
    echo "OTEL Agent容器不存在或已删除"
fi

echo "OTEL Agent服务已完全停止"

# === 统一数据卷删除处理 ===
SERVICE_NAME="OTEL Agent"
DATA_VOLUMES="${CONTAINER_NAME_OTEL_AGENT}-data"
...
if [ "$should_remove_data" = true ]; then
    ...
    echo ""
    echo "   删除数据的方式："
    echo "     环境变量: REMOVE_DATA=true make docker.otel-agent.stop"
    echo "     直接调用: bash <脚本路径> --remove-data"
    echo "     强制删除: REMOVE_DATA=true FORCE_DELETE=true make docker.otel-agent.stop"
fi