...
# 停止容器
if docker stop "${PROJ_PREFIX}-jaeger" 2>/dev/null; then
    echo "Jaeger容器已停止: ${PROJ_PREFIX}-jaeger"
else
    echo "Jaeger容器未运行或已停止"
fi

# 删除容器
if docker rm "${PROJ_PREFIX}-jaeger" 2>/dev/null; then
    echo "Jaeger容器已删除: ${PROJ_PREFIX}-jaeger"
else
    echo "Jaeger容器不存在或已删除"
fi
...
# === 统一数据卷删除处理 ===
SERVICE_NAME="Jaeger"
DATA_VOLUMES="${CONTAINER_NAME}-data"
DATA_DESCRIPTION="  - 追踪数据和索引
  - Span存储文件
  - 查询缓存
  - 临时追踪文件"
...
if [ "$should_remove_data" = true ]; then
    echo ""
    echo "⚠️  警告：将删除${SERVICE_NAME}数据卷！这将永久删除所有追踪数据！"
    echo "包括："
    ...
    echo ""
    echo "   删除数据的方式："
    echo "     环境变量: REMOVE_DATA=true make docker.jaeger.stop"
    echo "     直接调用: bash <脚本路径> --remove-data"
    echo "     强制删除: REMOVE_DATA=true FORCE_DELETE=true make docker.jaeger.stop"
fi