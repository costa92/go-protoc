...
readonly VOLUME_NAME="${CONTAINER_NAME_MYSQL}-data"
...
# === 统一数据卷删除处理 ===
SERVICE_NAME="MySQL"
DATA_VOLUMES="${CONTAINER_NAME_MYSQL}-data"
...
if [ "$should_remove_data" = true ]; then
    ...
    echo ""
    echo "   删除数据的方式："
    echo "     环境变量: REMOVE_DATA=true make docker.mysql.stop"
    echo "     直接调用: bash <脚本路径> --remove-data"
    echo "     强制删除: REMOVE_DATA=true FORCE_DELETE=true make docker.mysql.stop"
fi