...
readonly VOLUME_NAME="${CONTAINER_NAME_OTEL_COLLECTOR}-data"

echo "正在停止OTEL Collector容器..."

# 停止容器
if docker stop "${CONTAINER_NAME}" 2>/dev/null; then
    echo "OTEL Collector容器已停止: ${CONTAINER_NAME}"
else
    echo "OTEL Collector容器未运行或已停止"
fi

# 删除容器
if docker rm "${CONTAINER_NAME}" 2>/dev/null; then
    echo "OTEL Collector容器已删除: ${CONTAINER_NAME}"
else
    echo "OTEL Collector容器不存在或已删除"
fi

echo "OTEL Collector服务已完全停止"

# === 统一数据卷删除处理 ===
SERVICE_NAME="OTEL Collector"
DATA_VOLUMES="${CONTAINER_NAME_OTEL_COLLECTOR}-data"
DATA_DESCRIPTION="  - 遥测数据缓存
  - 处理器状态
  - 导出器队列
  - 配置文件备份
  - 统计和诊断数据"
...
if [ "$should_remove_data" = true ]; then
    ...
    echo ""
    echo "   删除数据的方式："
    echo "     环境变量: REMOVE_DATA=true make docker.otel-collector.stop"
    echo "     直接调用: bash <脚本路径> --remove-data"
    echo "     强制删除: REMOVE_DATA=true FORCE_DELETE=true make docker.otel-collector.stop"
fi