...
readonly VOLUME_NAME="${CONTAINER_NAME_ETCD}-data"
...
# === 统一数据卷删除处理 ===
SERVICE_NAME="etcd"
DATA_VOLUMES="${CONTAINER_NAME_ETCD}-data"
...
if [ "$should_remove_data" = true ]; then
    echo ""
    echo "⚠️  警告：将删除${SERVICE_NAME}数据卷！这将永久删除所有配置数据！"
    echo "包括："
    ...
    echo ""
    echo "   删除数据的方式："
    echo "     环境变量: REMOVE_DATA=true make docker.etcd.stop"
    echo "     直接调用: bash <脚本路径> --remove-data"
    echo "     强制删除: REMOVE_DATA=true FORCE_DELETE=true make docker.etcd.stop"
fi